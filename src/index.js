// From https://github.com/boosh/alpine-tour
document.addEventListener('alpine:init', () => {
    ...
    Alpine.directive('tour', (el, { modifiers, expression }, { evaluateLater, effect }) => {
        ...
        if (~['top', 'bottom', 'left', 'right'].indexOf(modifier)) {
            ...
            getContent((content) => {
                const existingStepIndex = tour.steps.findIndex(step => step.step === content.step);

                if (existingStepIndex !== -1) {
                    // Update the existing step with the new element
                    tour.steps[existingStepIndex] = {
                        step: content.step ?? 1,
                        title: content.title ?? '',
                    ...
                    tour.steps.sort((a, b) => a.step - b.step);
                }
            });
